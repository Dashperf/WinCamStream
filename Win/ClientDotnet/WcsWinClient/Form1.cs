...
public partial class Form1 : Form
{
    ...
    private void BuildUi()
    {
        ...
        _iproxyRestartButton = AddButton(controlsPanel, "Restart iProxy", 12, y, 116);
        _statusButton = AddButton(controlsPanel, "Status", 134, y, 76);
        _startButton = AddButton(controlsPanel, "Start", 216, y, 76);
        _stopButton = AddButton(controlsPanel, "Stop", 298, y, 76);
        _restartButton = AddButton(controlsPanel, "Restart", 380, y, 80);
    ...
    {
        _iproxyRestartButton.Click += (_, _) => StartIproxyTunnels();
        ...
        try
        {
            ...
            if (root.TryGetProperty("config", out var cfg))
            {
                ...
                else
                {
                    ...
                    {
                        Log("Remote config changed while local edits are pending. Click 'Apply' to push local values.");
                    }
